...
class PlayerSystem(
    ...
    override fun processEntity(entity: Entity, deltaTime: Float) {
        ...
        eventQueue.proceed { eventContext ->
            val isEvent = SPAWN == eventContext.eventType
            val isPlayer = eventContext.params.containsKey(playerName)
            val occurred = isPlayer && isEvent
            if (occurred) {
                playerBuilder.buildUnits(playerComponent.playerJson)
                    .onEach { it.add(engine) }
                    .onEach { playerComponent.units.add(it) }
            }
            occurred
        }
    }
