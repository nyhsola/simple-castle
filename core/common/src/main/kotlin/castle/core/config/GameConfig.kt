...
class GameConfig(commonConfig: CommonConfig) : Disposable {
    ...
    private val mapScanService = MapScanService(environmentService, physicService)
    private val mapService = MapService(engine, eventQueue, mapScanService, environmentService)
    private val uiService = UIService(engine, eventQueue, mapScanService, commonResources, shapeRenderer, spriteBatch, scalingViewport)
    ...
    private val unitSystem = UnitSystem(eventQueue, mapService, physicService)
    private val mapSystem = MapSystem(mapService)
    private val gameManagerSystem = GameManagerSystem(environmentService, gameService, uiService, selectionService, cameraService, mapScanService, mapService, eventQueue)
